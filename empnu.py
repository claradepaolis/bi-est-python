...
def PU_nested_em_opt(X_unlabeled, X_labeled_pos, X_labeled_neg, K, max_steps=2000, rnd_state=0,
    ...
    while continue_opt:
        ...
        for k in range(K):
            ...
            if N_labeled_pos > 0:
                ...
                mu[c, :, k] = mu[c, :, k] + (pos_scale * np.sum(pX, axis=0))
                pxmu = np.sqrt(posteriors_labeled[:, c, k])[:, np.newaxis] * xmu_labeled
                sg[c, :, :, k] = sg[c, :, :, k] + (
                        pos_scale * np.reshape(np.matmul(np.transpose(pxmu), pxmu), [dim, dim]))
            ...
            if N_labeled_neg > 0:
                ...
                mu[c, :, k] = mu[c, :, k] + (neg_scale * np.sum(pX, axis=0))
                pxmu = np.sqrt(posteriors_labeled_neg[:, c, k])[:, np.newaxis] * xmu_labeled_neg
                sg[c, :, :, k] = sg[c, :, :, k] + (
                        neg_scale * np.reshape(np.matmul(np.transpose(pxmu), pxmu), [dim, dim]))
    ...
    # reshape mu to num_classes (2) * num_components * dim
    mus = [mu[0].T, mu[1].T]
    # reshape sigma to num_classes (2) * num_components * dim * dim
    # during optimization, it is 2 * dim*dim*num_components
    covariance = [np.transpose(sg[0], axes=[2, 1, 0]), np.transpose(sg[1], axes=[2, 1, 0])]

    elapsed = time.time() - start_time
    return alphas[0], w, w_labeled, covariance, mus, lls
